...
var _IntegerBufferSet = _interopRequireDefault(
  require('./../vendor_upstream/struct/IntegerBufferSet')
);

var _PrefixIntervalTree = _interopRequireDefault(
  require('./../vendor_upstream/struct/PrefixIntervalTree')
);
...
var _convertColumnElementsToData = _interopRequireDefault(
  require('./../helper/convertColumnElementsToData')
);
...
var _columnStateHelper = _interopRequireDefault(
  require('././columnStateHelper')
);

var _computeRenderedRows = _interopRequireDefault(
  require('././computeRenderedRows')
);
...
function reducers() {
  ...
  switch (action.type) {
    ...
    case ActionTypes.PROP_CHANGE: {
      ...
      var _scrollAnchor = (0, _scrollAnchor3.getScrollAnchor)(
        _newState,
        newProps,
        oldProps
      ...
      if (
        !(0, _shallowEqual['default'])(state, _newState) ||
        _scrollAnchor.changed
      ) {
        _newState = (0, _computeRenderedRows['default'])(
          _newState,
          _scrollAnchor
        );
      }

      _newState = _columnStateHelper['default'].initialize(
        _newState,
        newProps,
        oldProps
      ...
      if (
        _newState.scrollX !== state.scrollX ||
        _newState.scrollY !== state.scrollY
      ) {
    ...
    case ActionTypes.SCROLL_END: {
      ...
      return (0, _computeRenderedRows['default'])(
        _newState2,
...
function initializeRowHeightsAndOffsets(state) {
  ...
  var rowOffsetIntervalTree = _PrefixIntervalTree['default'].uniform(
    rowsCount,
...
function setStateFromProps(state, props) {
  var _convertColumnElement = (0, _convertColumnElementsToData['default'])(
      props.children
    ),
  ...
  newState.elementHeights = _extends(
    {},
    newState.elementHeights,
    (0, _pick['default'])(props, [
